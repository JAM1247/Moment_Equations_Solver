...
# Main function to compute the Fokker Planck and graph stuff related toit
# I seperated the mains so that when i have to chagne it to your fokker planck solver, it should be a much more straight forward process
def main():
    ...
    # Extractting the momentss so we can plot special moments over time
    arr_Ex = []
    arr_Ey = []
    arr_Ex2 = []
    arr_Ey2 = []
...
# I'm just solving using Forward Eulor Scheme, felt like it sufficed but I suppose if we want to get even more accurate we might have to change it
...
def compare_moment_methods():
    ...
    print("===== Compare PDE-based Moments vs. Naive ODE-based Moments (with error) =====")
    D = 0.2
    dt = 1e-5
    T = 0.005
    Nx, Ny = 50, 50
    ...
    n_steps_pde = len(moments_list)
    t_array_pde = np.linspace(0, T, n_steps_pde)

    pde_Ex, pde_Ey, pde_Ex2, pde_Ey2, pde_Exy = [], [], [], [], []
    for mdict in moments_list:
    ...
    # Bug fix, have to make sure the arrays are the same length (causing some issues earliers)
    if len(M_ode) > n_steps_pde:
        M_ode = M_ode[:n_steps_pde]
        t_ode = t_ode[:n_steps_pde]
    ...
    # Comparing the raw moments via graphing 
    plt.figure(figsize=(10,6))
    plt.suptitle("Comparison: PDE-based Moments vs. Naive ODE (Closure)")

    plt.subplot(2,3,1)
    plt.plot(t_array_pde, pde_Ex, 'b-', label='PDE E[x]')
    plt.plot(t_ode,       ode_Ex, 'r--', label='ODE E[x]')
    plt.title("E[x](t)")
    plt.grid(True)
    plt.legend()

    plt.subplot(2,3,2)
    plt.plot(t_array_pde, pde_Ey, 'b-', label='PDE E[y]')
    plt.plot(t_ode,       ode_Ey, 'r--', label='ODE E[y]')
    plt.title("E[y](t)")
    plt.grid(True)

    plt.subplot(2,3,3)
    plt.plot(t_array_pde, pde_Ex2, 'b-', label='PDE E[x^2]')
    plt.plot(t_ode,       ode_Ex2, 'r--', label='ODE E[x^2]')
    plt.title("E[x^2](t)")
    plt.grid(True)

    plt.subplot(2,3,4)
    plt.plot(t_array_pde, pde_Ey2, 'b-', label='PDE E[y^2]')
    plt.plot(t_ode,       ode_Ey2, 'r--', label='ODE E[y^2]')
    plt.title("E[y^2](t)")
    plt.grid(True)

    plt.subplot(2,3,5)
    plt.plot(t_array_pde, pde_Exy, 'b-', label='PDE E[x y]')
    plt.plot(t_ode,       ode_Exy, 'r--', label='ODE E[x y]')
    plt.title("E[x y](t)")
    plt.grid(True)

    plt.tight_layout()
    plt.savefig("compare_pde_vs_naive_moments.png", dpi=300)
    plt.close()
    print("Saved 'compare_pde_vs_naive_moments.png'.")





    # Relative error calculation and grpahs
    eps = 1e-12
    ...
    plt.figure(figsize=(10,6))
    plt.suptitle("Relative Errors: PDE vs. Naive ODE Moments")

    plt.subplot(2,3,1)
    plt.plot(t_array_pde, rel_err_Ex, '-')
    plt.title("Rel. Error in E[x]")
    plt.yscale('log')
    plt.grid(True)

    plt.subplot(2,3,2)
    plt.plot(t_array_pde, rel_err_Ey, '-')
    plt.title("Rel. Error in E[y]")
    plt.yscale('log')
    plt.grid(True)

    plt.subplot(2,3,3)
    plt.plot(t_array_pde, rel_err_Ex2, '-')
    plt.title("Rel. Error in E[x^2]")
    plt.yscale('log')
    plt.grid(True)

    plt.subplot(2,3,4)
    plt.plot(t_array_pde, rel_err_Ey2, '-')
    plt.title("Rel. Error in E[y^2]")
    plt.yscale('log')
    plt.grid(True)

    plt.subplot(2,3,5)
    plt.plot(t_array_pde, rel_err_Exy, '-')
    plt.title("Rel. Error in E[x y]")
    plt.yscale('log')
    plt.grid(True)

    plt.tight_layout()
    plt.savefig("relative_error_pde_vs_ode.png", dpi=300)
    plt.close()
    print("Saved 'relative_error_pde_vs_ode.png'.")

    print("===== SIMULATION FINIHED =-====")
